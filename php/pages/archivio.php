<!DOCTYPE html>
<html lang="en" dir="ltr">
  <head>
    <meta charset="utf-8">
    <title>Unifichiamoci</title>
    <link rel="stylesheet" href="../../css/studioscopio.css">
    <link href="../../css/style.css" rel="stylesheet"type="text/css">
  </head>
  <body style="margin: 0;">
    <?php
  	echo file_get_contents("../header.php");
  	 ?>
    <div class="centering">
      <div class="main_container">
        <div class="article_container">
          <div class="image_container">
            <a href="vita_universitaria.html"><img src="../../images/studioscopio_1.webp" class="image_contained" style="height: 529px; width: 940px;"></a>
          </div>
          <div class="didascalia">
            <div class="author_container">
              <div class="svg_image_account">
                <a href="#">
                  <svg width="32" height="32" viewBox="0 0 1000 1000" class="HOi00">
                    <circle cx="500" cy="500" r="500" fill="#cccccc"></circle>
                    <path fill="#a0a09f" d="M830.8,874.927c-77.344-60.8-187.181-104.877-227.88-111.347-20.335-3.233-20.8-59.1-20.8-59.1s59.746-59.106,72.768-138.584c35.029,0,56.666-84.5,21.631-114.226C677.986,420.37,721.551,206,501,206S324.015,420.37,325.473,451.666c-35.033,29.729-13.4,114.226,21.632,114.226,13.021,79.478,72.77,138.584,72.77,138.584s-0.464,55.871-20.8,59.1c-40.883,6.5-151.537,50.943-228.934,112.176C65.84,784.12,0,649.751,0,500,0,223.858,223.858,0,500,0s500,223.858,500,500C1000,649.3,934.559,783.311,830.8,874.927ZM500,1000h0Z"></path>
                  </svg>
                </a>
              </div>
              <div class="author">
                <div class="author_name">
                  <div class="name">
                    <a href="#" class="nocolor">
                      <span>Laura Nortilli</span>
                    </a>
                  </div>
                  <div class="svg_image_profile">
                    <a href="#">
                      <svg xmlns="http://www.w3.org/2000/svg" width="19" height="19" viewBox="0 0 19 19" class="blog-post-post-list-description-fill" style="fill-rule: evenodd;">
                        <path d="M3.0299,4.05263158 C7.59702941,1.31578947 11.4029706,1.31578947 15.9701,4.05263158 C16.0788412,11 16.2419529,14.3157895 9.5,17 C2.75804706,14.3157895 2.92115882,11 3.0299,4.05263158 Z M9.39332888,11.2792808 C9.45851262,11.2381732 9.54151575,11.2381732 9.60669949,11.2792808 L11.6016734,12.5373957 C11.7030093,12.6013024 11.8276546,12.6164227 11.9413311,12.5785984 C12.1509459,12.508852 12.2643317,12.2823849 12.1945852,12.07277 L11.4749606,9.91002155 L13.4066196,8.57961581 C13.5127893,8.50649273 13.5772092,8.38668249 13.5796591,8.25779106 C13.5838574,8.03691706 13.408207,7.85445994 13.1873329,7.85026169 L10.7205982,7.80337534 L9.87345622,5.59604095 C9.83283079,5.4901863 9.74919031,5.40654582 9.64333566,5.36592038 C9.43708931,5.28676612 9.20572641,5.38979461 9.12657215,5.59604095 L8.27943019,7.80337534 L5.81269542,7.85026169 C5.68380399,7.85271159 5.56399375,7.91713145 5.49087067,8.02330121 C5.36556375,8.20523836 5.41147163,8.45430889 5.59340879,8.57961581 L7.52506779,9.91002155 L6.80544312,12.07277 C6.76761883,12.1864466 6.78273912,12.3110919 6.84664582,12.4124278 C6.96448715,12.599287 7.21149576,12.655237 7.39835499,12.5373957 L9.39332888,11.2792808 Z">
                        </path>
                      </svg>
                    </a>
                  </div>
                </div>
                <div class="date_and_time">
                  <div>
                    <span>lug29</span>
                  </div>
                  <div class="point_container">
                    <div class="point"></div>
                  </div>
                  <div>
                    <span>8min</span>
                  </div>
              </div>
              </div>
            </div>
            <a href="#" class="nocolor" id="article_refer">
              <div class="description">
                <div class="title">
                  OROSCOPO AGOSTO 2021
                </div>
                <div class="description_text">
                  DATE CHIAVE ED EVENTI ASTROLOGICI - 1 agosto: Marte in quadratura con Lilith - 8 agosto: Luna nuova in Leone - 11 agosto: Mercurio entra...
                </div>
              </div>
            </a>
          </div>
        </div>
        <div class="article_container">
          <div class="image_container">
            <a href="vita_universitaria.html"><img src="../../images/studioscopio_1.webp" class="image_contained" style="height: 529px; width: 940px;"></a>
          </div>
          <div class="didascalia">
            <div class="author_container">
              <div class="svg_image_account">
                <a href="#">
                  <svg width="32" height="32" viewBox="0 0 1000 1000" class="HOi00">
                    <circle cx="500" cy="500" r="500" fill="#cccccc"></circle>
                    <path fill="#a0a09f" d="M830.8,874.927c-77.344-60.8-187.181-104.877-227.88-111.347-20.335-3.233-20.8-59.1-20.8-59.1s59.746-59.106,72.768-138.584c35.029,0,56.666-84.5,21.631-114.226C677.986,420.37,721.551,206,501,206S324.015,420.37,325.473,451.666c-35.033,29.729-13.4,114.226,21.632,114.226,13.021,79.478,72.77,138.584,72.77,138.584s-0.464,55.871-20.8,59.1c-40.883,6.5-151.537,50.943-228.934,112.176C65.84,784.12,0,649.751,0,500,0,223.858,223.858,0,500,0s500,223.858,500,500C1000,649.3,934.559,783.311,830.8,874.927ZM500,1000h0Z"></path>
                  </svg>
                </a>
              </div>
              <div class="author">
                <div class="author_name">
                  <div class="name">
                    <a href="#" class="nocolor">
                      <span>-</span>
                    </a>
                  </div>
                  <div class="svg_image_profile">
                    <a href="#">
                      <svg xmlns="http://www.w3.org/2000/svg" width="19" height="19" viewBox="0 0 19 19" class="blog-post-post-list-description-fill" style="fill-rule: evenodd;">
                        <path d="M3.0299,4.05263158 C7.59702941,1.31578947 11.4029706,1.31578947 15.9701,4.05263158 C16.0788412,11 16.2419529,14.3157895 9.5,17 C2.75804706,14.3157895 2.92115882,11 3.0299,4.05263158 Z M9.39332888,11.2792808 C9.45851262,11.2381732 9.54151575,11.2381732 9.60669949,11.2792808 L11.6016734,12.5373957 C11.7030093,12.6013024 11.8276546,12.6164227 11.9413311,12.5785984 C12.1509459,12.508852 12.2643317,12.2823849 12.1945852,12.07277 L11.4749606,9.91002155 L13.4066196,8.57961581 C13.5127893,8.50649273 13.5772092,8.38668249 13.5796591,8.25779106 C13.5838574,8.03691706 13.408207,7.85445994 13.1873329,7.85026169 L10.7205982,7.80337534 L9.87345622,5.59604095 C9.83283079,5.4901863 9.74919031,5.40654582 9.64333566,5.36592038 C9.43708931,5.28676612 9.20572641,5.38979461 9.12657215,5.59604095 L8.27943019,7.80337534 L5.81269542,7.85026169 C5.68380399,7.85271159 5.56399375,7.91713145 5.49087067,8.02330121 C5.36556375,8.20523836 5.41147163,8.45430889 5.59340879,8.57961581 L7.52506779,9.91002155 L6.80544312,12.07277 C6.76761883,12.1864466 6.78273912,12.3110919 6.84664582,12.4124278 C6.96448715,12.599287 7.21149576,12.655237 7.39835499,12.5373957 L9.39332888,11.2792808 Z">
                        </path>
                      </svg>
                    </a>
                  </div>
                </div>
                <div class="date_and_time">
                  <div>
                    <span>lug29</span>
                  </div>
                  <div class="point_container">
                    <div class="point"></div>
                  </div>
                  <div>
                    <span>8min</span>
                  </div>
              </div>
            </div>
            </div>
            <a href="#" class="nocolor" id="article_refer">
              <div class="description">
                <div class="title">
                  OROSCOPO GIUGNO 2021
                </div>
                <div class="description_text">
                  DATE CHIAVE ED EVENTI ASTROLOGICI - 1 agosto: Marte in quadratura con Lilith - 8 agosto: Luna nuova in Leone - 11 agosto: Mercurio entra...
                </div>
              </div>
            </a>
          </div>
        </div>
        <div class="article_container">
          <div class="image_container">
            <a href="vita_universitaria.html"><img src="../../images/studioscopio_1.webp" class="image_contained" style="height: 529px; width: 940px;"></a>
          </div>
          <div class="didascalia">
            <div class="author_container">
              <div class="svg_image_account">
                <a href="#">
                  <svg width="32" height="32" viewBox="0 0 1000 1000" class="HOi00">
                    <circle cx="500" cy="500" r="500" fill="#cccccc"></circle>
                    <path fill="#a0a09f" d="M830.8,874.927c-77.344-60.8-187.181-104.877-227.88-111.347-20.335-3.233-20.8-59.1-20.8-59.1s59.746-59.106,72.768-138.584c35.029,0,56.666-84.5,21.631-114.226C677.986,420.37,721.551,206,501,206S324.015,420.37,325.473,451.666c-35.033,29.729-13.4,114.226,21.632,114.226,13.021,79.478,72.77,138.584,72.77,138.584s-0.464,55.871-20.8,59.1c-40.883,6.5-151.537,50.943-228.934,112.176C65.84,784.12,0,649.751,0,500,0,223.858,223.858,0,500,0s500,223.858,500,500C1000,649.3,934.559,783.311,830.8,874.927ZM500,1000h0Z"></path>
                  </svg>
                </a>
              </div>
              <div class="author">
                <div class="author_name">
                  <div class="name">
                    <a href="#" class="nocolor">
                      <span>Laura Nortilli</span>
                    </a>
                  </div>
                  <div class="svg_image_profile">
                    <a href="#">
                      <svg xmlns="http://www.w3.org/2000/svg" width="19" height="19" viewBox="0 0 19 19" class="blog-post-post-list-description-fill" style="fill-rule: evenodd;">
                        <path d="M3.0299,4.05263158 C7.59702941,1.31578947 11.4029706,1.31578947 15.9701,4.05263158 C16.0788412,11 16.2419529,14.3157895 9.5,17 C2.75804706,14.3157895 2.92115882,11 3.0299,4.05263158 Z M9.39332888,11.2792808 C9.45851262,11.2381732 9.54151575,11.2381732 9.60669949,11.2792808 L11.6016734,12.5373957 C11.7030093,12.6013024 11.8276546,12.6164227 11.9413311,12.5785984 C12.1509459,12.508852 12.2643317,12.2823849 12.1945852,12.07277 L11.4749606,9.91002155 L13.4066196,8.57961581 C13.5127893,8.50649273 13.5772092,8.38668249 13.5796591,8.25779106 C13.5838574,8.03691706 13.408207,7.85445994 13.1873329,7.85026169 L10.7205982,7.80337534 L9.87345622,5.59604095 C9.83283079,5.4901863 9.74919031,5.40654582 9.64333566,5.36592038 C9.43708931,5.28676612 9.20572641,5.38979461 9.12657215,5.59604095 L8.27943019,7.80337534 L5.81269542,7.85026169 C5.68380399,7.85271159 5.56399375,7.91713145 5.49087067,8.02330121 C5.36556375,8.20523836 5.41147163,8.45430889 5.59340879,8.57961581 L7.52506779,9.91002155 L6.80544312,12.07277 C6.76761883,12.1864466 6.78273912,12.3110919 6.84664582,12.4124278 C6.96448715,12.599287 7.21149576,12.655237 7.39835499,12.5373957 L9.39332888,11.2792808 Z">
                        </path>
                      </svg>
                    </a>
                  </div>
                </div>
                <div class="date_and_time">
                  <div>
                    <span>lug29</span>
                  </div>
                  <div class="point_container">
                    <div class="point"></div>
                  </div>
                  <div>
                    <span>8min</span>
                  </div>
              </div>
            </div>
            </div>
            <a href="#" class="nocolor" id="article_refer">
              <div class="description">
                <div class="title">
                  OROSCOPO AGOSTO 2021
                </div>
                <div class="description_text">
                  DATE CHIAVE ED EVENTI ASTROLOGICI - 1 agosto: Marte in quadratura con Lilith - 8 agosto: Luna nuova in Leone - 11 agosto: Mercurio entra...
                </div>
              </div>
            </a>
          </div>
        </div>
        <div class="article_container">
          <div class="image_container">
            <a href="vita_universitaria.html"><img src="../../images/studioscopio_1.webp" class="image_contained" style="height: 529px; width: 940px;"></a>
          </div>
          <div class="didascalia">
            <div class="author_container">
              <div class="svg_image_account">
                <a href="#">
                  <svg width="32" height="32" viewBox="0 0 1000 1000" class="HOi00">
                    <circle cx="500" cy="500" r="500" fill="#cccccc"></circle>
                    <path fill="#a0a09f" d="M830.8,874.927c-77.344-60.8-187.181-104.877-227.88-111.347-20.335-3.233-20.8-59.1-20.8-59.1s59.746-59.106,72.768-138.584c35.029,0,56.666-84.5,21.631-114.226C677.986,420.37,721.551,206,501,206S324.015,420.37,325.473,451.666c-35.033,29.729-13.4,114.226,21.632,114.226,13.021,79.478,72.77,138.584,72.77,138.584s-0.464,55.871-20.8,59.1c-40.883,6.5-151.537,50.943-228.934,112.176C65.84,784.12,0,649.751,0,500,0,223.858,223.858,0,500,0s500,223.858,500,500C1000,649.3,934.559,783.311,830.8,874.927ZM500,1000h0Z"></path>
                  </svg>
                </a>
              </div>
              <div class="author">
                <div class="author_name">
                  <div class="name">
                    <a href="#" class="nocolor">
                      <span>Laura Nortilli</span>
                    </a>
                  </div>
                  <div class="svg_image_profile">
                    <a href="#">
                      <svg xmlns="http://www.w3.org/2000/svg" width="19" height="19" viewBox="0 0 19 19" class="blog-post-post-list-description-fill" style="fill-rule: evenodd;">
                        <path d="M3.0299,4.05263158 C7.59702941,1.31578947 11.4029706,1.31578947 15.9701,4.05263158 C16.0788412,11 16.2419529,14.3157895 9.5,17 C2.75804706,14.3157895 2.92115882,11 3.0299,4.05263158 Z M9.39332888,11.2792808 C9.45851262,11.2381732 9.54151575,11.2381732 9.60669949,11.2792808 L11.6016734,12.5373957 C11.7030093,12.6013024 11.8276546,12.6164227 11.9413311,12.5785984 C12.1509459,12.508852 12.2643317,12.2823849 12.1945852,12.07277 L11.4749606,9.91002155 L13.4066196,8.57961581 C13.5127893,8.50649273 13.5772092,8.38668249 13.5796591,8.25779106 C13.5838574,8.03691706 13.408207,7.85445994 13.1873329,7.85026169 L10.7205982,7.80337534 L9.87345622,5.59604095 C9.83283079,5.4901863 9.74919031,5.40654582 9.64333566,5.36592038 C9.43708931,5.28676612 9.20572641,5.38979461 9.12657215,5.59604095 L8.27943019,7.80337534 L5.81269542,7.85026169 C5.68380399,7.85271159 5.56399375,7.91713145 5.49087067,8.02330121 C5.36556375,8.20523836 5.41147163,8.45430889 5.59340879,8.57961581 L7.52506779,9.91002155 L6.80544312,12.07277 C6.76761883,12.1864466 6.78273912,12.3110919 6.84664582,12.4124278 C6.96448715,12.599287 7.21149576,12.655237 7.39835499,12.5373957 L9.39332888,11.2792808 Z">
                        </path>
                      </svg>
                    </a>
                  </div>
                </div>
                <div class="date_and_time">
                  <div>
                    <span>lug29</span>
                  </div>
                  <div class="point_container">
                    <div class="point"></div>
                  </div>
                  <div>
                    <span>8min</span>
                  </div>
              </div>
            </div>
            </div>
            <a href="#" class="nocolor" id="article_refer">
              <div class="description">
                <div class="title">
                  OROSCOPO AGOSTO 2021
                </div>
                <div class="description_text">
                  DATE CHIAVE ED EVENTI ASTROLOGICI - 1 agosto: Marte in quadratura con Lilith - 8 agosto: Luna nuova in Leone - 11 agosto: Mercurio entra...
                </div>
              </div>
            </a>
          </div>
        </div>
        <div class="article_container">
          <div class="image_container">
            <a href="vita_universitaria.html"><img src="../../images/studioscopio_1.webp" class="image_contained" style="height: 529px; width: 940px;"></a>
          </div>
          <div class="didascalia">
            <div class="author_container">
              <div class="svg_image_account">
                <a href="#">
                  <svg width="32" height="32" viewBox="0 0 1000 1000" class="HOi00">
                    <circle cx="500" cy="500" r="500" fill="#cccccc"></circle>
                    <path fill="#a0a09f" d="M830.8,874.927c-77.344-60.8-187.181-104.877-227.88-111.347-20.335-3.233-20.8-59.1-20.8-59.1s59.746-59.106,72.768-138.584c35.029,0,56.666-84.5,21.631-114.226C677.986,420.37,721.551,206,501,206S324.015,420.37,325.473,451.666c-35.033,29.729-13.4,114.226,21.632,114.226,13.021,79.478,72.77,138.584,72.77,138.584s-0.464,55.871-20.8,59.1c-40.883,6.5-151.537,50.943-228.934,112.176C65.84,784.12,0,649.751,0,500,0,223.858,223.858,0,500,0s500,223.858,500,500C1000,649.3,934.559,783.311,830.8,874.927ZM500,1000h0Z"></path>
                  </svg>
                </a>
              </div>
              <div class="author">
                <div class="author_name">
                  <div class="name">
                    <a href="#" class="nocolor">
                      <span>Laura Nortilli</span>
                    </a>
                  </div>
                  <div class="svg_image_profile">
                    <a href="#">
                      <svg xmlns="http://www.w3.org/2000/svg" width="19" height="19" viewBox="0 0 19 19" class="blog-post-post-list-description-fill" style="fill-rule: evenodd;">
                        <path d="M3.0299,4.05263158 C7.59702941,1.31578947 11.4029706,1.31578947 15.9701,4.05263158 C16.0788412,11 16.2419529,14.3157895 9.5,17 C2.75804706,14.3157895 2.92115882,11 3.0299,4.05263158 Z M9.39332888,11.2792808 C9.45851262,11.2381732 9.54151575,11.2381732 9.60669949,11.2792808 L11.6016734,12.5373957 C11.7030093,12.6013024 11.8276546,12.6164227 11.9413311,12.5785984 C12.1509459,12.508852 12.2643317,12.2823849 12.1945852,12.07277 L11.4749606,9.91002155 L13.4066196,8.57961581 C13.5127893,8.50649273 13.5772092,8.38668249 13.5796591,8.25779106 C13.5838574,8.03691706 13.408207,7.85445994 13.1873329,7.85026169 L10.7205982,7.80337534 L9.87345622,5.59604095 C9.83283079,5.4901863 9.74919031,5.40654582 9.64333566,5.36592038 C9.43708931,5.28676612 9.20572641,5.38979461 9.12657215,5.59604095 L8.27943019,7.80337534 L5.81269542,7.85026169 C5.68380399,7.85271159 5.56399375,7.91713145 5.49087067,8.02330121 C5.36556375,8.20523836 5.41147163,8.45430889 5.59340879,8.57961581 L7.52506779,9.91002155 L6.80544312,12.07277 C6.76761883,12.1864466 6.78273912,12.3110919 6.84664582,12.4124278 C6.96448715,12.599287 7.21149576,12.655237 7.39835499,12.5373957 L9.39332888,11.2792808 Z">
                        </path>
                      </svg>
                    </a>
                  </div>
                </div>
                <div class="date_and_time">
                  <div>
                    <span>lug29</span>
                  </div>
                  <div class="point_container">
                    <div class="point"></div>
                  </div>
                  <div>
                    <span>8min</span>
                  </div>
              </div>
            </div>
            </div>
            <a href="#" class="nocolor" id="article_refer">
              <div class="description">
                <div class="title">
                  OROSCOPO AGOSTO 2021
                </div>
                <div class="description_text">
                  DATE CHIAVE ED EVENTI ASTROLOGICI - 1 agosto: Marte in quadratura con Lilith - 8 agosto: Luna nuova in Leone - 11 agosto: Mercurio entra...
                </div>
              </div>
            </a>
          </div>
        </div>
        <div class="article_container">
          <div class="image_container">
            <a href="vita_universitaria.html"><img src="../../images/studioscopio_1.webp" class="image_contained" style="height: 529px; width: 940px;"></a>
          </div>
          <div class="didascalia">
            <div class="author_container">
              <div class="svg_image_account">
                <a href="#">
                  <svg width="32" height="32" viewBox="0 0 1000 1000" class="HOi00">
                    <circle cx="500" cy="500" r="500" fill="#cccccc"></circle>
                    <path fill="#a0a09f" d="M830.8,874.927c-77.344-60.8-187.181-104.877-227.88-111.347-20.335-3.233-20.8-59.1-20.8-59.1s59.746-59.106,72.768-138.584c35.029,0,56.666-84.5,21.631-114.226C677.986,420.37,721.551,206,501,206S324.015,420.37,325.473,451.666c-35.033,29.729-13.4,114.226,21.632,114.226,13.021,79.478,72.77,138.584,72.77,138.584s-0.464,55.871-20.8,59.1c-40.883,6.5-151.537,50.943-228.934,112.176C65.84,784.12,0,649.751,0,500,0,223.858,223.858,0,500,0s500,223.858,500,500C1000,649.3,934.559,783.311,830.8,874.927ZM500,1000h0Z"></path>
                  </svg>
                </a>
              </div>
              <div class="author">
                <div class="author_name">
                  <div class="name">
                    <a href="#" class="nocolor">
                      <span>Laura Nortilli</span>
                    </a>
                  </div>
                  <div class="svg_image_profile">
                    <a href="#">
                      <svg xmlns="http://www.w3.org/2000/svg" width="19" height="19" viewBox="0 0 19 19" class="blog-post-post-list-description-fill" style="fill-rule: evenodd;">
                        <path d="M3.0299,4.05263158 C7.59702941,1.31578947 11.4029706,1.31578947 15.9701,4.05263158 C16.0788412,11 16.2419529,14.3157895 9.5,17 C2.75804706,14.3157895 2.92115882,11 3.0299,4.05263158 Z M9.39332888,11.2792808 C9.45851262,11.2381732 9.54151575,11.2381732 9.60669949,11.2792808 L11.6016734,12.5373957 C11.7030093,12.6013024 11.8276546,12.6164227 11.9413311,12.5785984 C12.1509459,12.508852 12.2643317,12.2823849 12.1945852,12.07277 L11.4749606,9.91002155 L13.4066196,8.57961581 C13.5127893,8.50649273 13.5772092,8.38668249 13.5796591,8.25779106 C13.5838574,8.03691706 13.408207,7.85445994 13.1873329,7.85026169 L10.7205982,7.80337534 L9.87345622,5.59604095 C9.83283079,5.4901863 9.74919031,5.40654582 9.64333566,5.36592038 C9.43708931,5.28676612 9.20572641,5.38979461 9.12657215,5.59604095 L8.27943019,7.80337534 L5.81269542,7.85026169 C5.68380399,7.85271159 5.56399375,7.91713145 5.49087067,8.02330121 C5.36556375,8.20523836 5.41147163,8.45430889 5.59340879,8.57961581 L7.52506779,9.91002155 L6.80544312,12.07277 C6.76761883,12.1864466 6.78273912,12.3110919 6.84664582,12.4124278 C6.96448715,12.599287 7.21149576,12.655237 7.39835499,12.5373957 L9.39332888,11.2792808 Z">
                        </path>
                      </svg>
                    </a>
                  </div>
                </div>
                <div class="date_and_time">
                  <div>
                    <span>lug29</span>
                  </div>
                  <div class="point_container">
                    <div class="point"></div>
                  </div>
                  <div>
                    <span>8min</span>
                  </div>
              </div>
            </div>
            </div>
            <a href="#" class="nocolor" id="article_refer">
              <div class="description">
                <div class="title">
                  OROSCOPO AGOSTO 2021
                </div>
                <div class="description_text">
                  DATE CHIAVE ED EVENTI ASTROLOGICI - 1 agosto: Marte in quadratura con Lilith - 8 agosto: Luna nuova in Leone - 11 agosto: Mercurio entra...
                </div>
              </div>
            </a>
          </div>
        </div>
      </div>
    </div>
    <iframe src="../footer.html" width="100%" height="142px;" style="border: none; margin-top: 40px;"></iframe>
    <script src="https://code.jquery.com/jquery-3.5.1.slim.min.js" integrity="sha384-DfXdz2htPH0lsSSs5nCTpuj/zy4C+OGpamoFVy38MVBnE+IbbVYUew+OrCXaRkfj" crossorigin="anonymous"></script>
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@4.5.3/dist/js/bootstrap.bundle.min.js" integrity="sha384-ho+j7jyWK8fNQe+A12Hb8AhRq26LrZ/JpcUGGOn+Y7RsweNrtN/tE3MoK7ZeZDyx" crossorigin="anonymous"></script></div>
  </body>
</html>
